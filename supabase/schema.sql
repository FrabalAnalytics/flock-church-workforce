...
-- First-timer care is intentionally separate from aggregate congregation
-- attendance because it contains identifiable contact and follow-up records.
...
begin
  ...
  select current_church_id, current_branch_id, service_uuid, selected.id
  from (select distinct id from unnest(p_department_ids) as chosen(id)) as selected
...
-- Normalize first-timer identity and consent state at the database boundary.
create or replace function public.prepare_first_timer_record()
returns trigger
language plpgsql
security definer
...
begin
  new.full_name := trim(new.full_name);
  new.phone_number := trim(new.phone_number);
  new.phone_number_normalized := regexp_replace(
    new.phone_number, '[^0-9]', '', 'g'
  );
  new.email := nullif(lower(trim(coalesce(new.email, ''))), '');
  new.updated_at := now();
...
drop trigger if exists prepare_first_timer_interaction
  on public.first_timer_interactions;
...
-- Return only presentation fields for a published programme whose revocable
-- public link is enabled and has not expired.
...
begin
  ...
  loop
    ...
    if not exists (select 1 from public.departments where id = worker_department_id) then
    ...
    insert into public.workers (
      full_name, phone_number, sex, department_id,
      status, joined_at, whatsapp_opt_in
    ) values (
      worker_name, worker_phone, worker_sex, worker_department_id,
      worker_status, worker_joined_at, worker_whatsapp_opt_in
    );
...
-- Delete another privileged login without leaving an active Auth account.
-- Historical records remain, and profile deletion is captured by the audit
-- trigger below when this snapshot is installed from scratch.
...
-- Programme-share tokens are secrets, so audit their lifecycle without
-- copying the token itself into immutable history.
...
drop trigger if exists audit_first_timer_interactions
  on public.first_timer_interactions;
...
grant select, insert, update, delete on table public.service_programme_shares to authenticated;
