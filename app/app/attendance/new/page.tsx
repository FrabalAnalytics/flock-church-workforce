...
export default async function NewAttendancePage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const [departmentResult, workersResult, submissionsResult] = await Promise.all([
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <WorkspaceNotice error={params.error ?? departmentResult.error?.message ?? workersResult.error?.message ?? submissionsResult.error?.message} />
      ...
      <AttendanceForm workers={workers ?? []} submittedServiceTypes={submittedServiceTypes} />
    </div>
  );
}
