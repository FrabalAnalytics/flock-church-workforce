...
import { ChurchAttendanceCorrectionForm } from "@/components/church-attendance-correction-form";
...
export default async function ChurchAttendancePage({
  ...
  const returnTo = `/app/church-attendance?${exportParams}`;

  return (
    <div className="mx-auto max-w-7xl">
      <WorkspaceNotice message={params.message} error={params.error ?? error?.message} />
      <PageHeader eyebrow="Congregation" title="Congregation attendance" description="Monitor aggregate congregation totals without collecting attendee identities." actions={<a href={`/api/reports/church-attendance.csv?${exportParams}`} className="flex min-h-12 w-full items-center justify-center rounded-xl border border-[var(--color-border)] bg-white px-5 text-sm font-semibold text-[var(--color-text-secondary)] shadow-[var(--shadow-sm)] hover:bg-[var(--color-surface-subtle)] sm:w-auto">Export CSV</a>} />
      <div className="mt-6 flex flex-wrap gap-2"><MetricPill value={rows.length} label="services" /><MetricPill value={total} label="total attendance" /><MetricPill value={average} label="average per service" /><MetricPill value={newMembers + newConverts} label="first steps" tone={(newMembers + newConverts) > 0 ? "warning" : "neutral"} /></div>

      <nav aria-label="Congregation attendance sections" className="mt-6 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none]">
        {profile.role === "super_admin" && <a href="#record-service" className="flex min-h-11 shrink-0 items-center rounded-xl bg-[var(--color-primary)] px-4 text-sm font-semibold text-white">Record service</a>}
        <a href="#attendance-insights" className="flex min-h-11 shrink-0 items-center rounded-xl border border-[var(--color-border)] bg-white px-4 text-sm font-semibold text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-subtle)]">Insights</a>
      ...
      {profile.role === "super_admin" ? <section id="record-service" className="mt-7 scroll-mt-24 rounded-3xl border border-[#dbe3f2] bg-white p-5 sm:p-6">
      ...
      <div id="attendance-insights" className="mt-6 grid scroll-mt-24 gap-4 sm:grid-cols-2 xl:grid-cols-5">
