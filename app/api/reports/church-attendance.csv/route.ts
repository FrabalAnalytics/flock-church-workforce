...
export async function GET(request: Request) {
  ...
  let query = supabase
    ...
    .select("adult_male_count, adult_female_count, children_count, total_count, updated_at, services!inner(service_date, service_type)")
  ...
  const header = ["Service date", "Service type", "Adult male", "Adult female", "Children", "Total", "Updated at"];
  const lines = (data ?? []).map((row) => {
    const serviceRow = row.services as unknown as { service_date: string; service_type: string } | null;
    return [serviceRow?.service_date, serviceRow?.service_type, row.adult_male_count, row.adult_female_count, row.children_count, row.total_count, row.updated_at].map(csvCell).join(",");
  });
